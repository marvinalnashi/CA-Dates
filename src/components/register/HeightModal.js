...
class HeightModal extends Component {
    ...
    renderItem = ({ item }) => {
        ...
        let textStyle = {...styles.itemText, fontWeight: selected ? '600' : '500', color: selected ? theme.blueColor : theme.secondaryColor};
        return (
            <TouchableFeedback onPress={()=>this.onHeightPress(item)}>
                <View style={[styles.itemView, {borderColor: theme.borderColor}]}>
                    <Text style={textStyle}>{item.title}</Text>
                    {selected && <Icon type={'feather'} name={'check'} size={20} color={theme.blueColor} style={{fontSize: 20, color: theme.blueColor}}/>}
                    ...
                    <View style={[styles.itemView, {marginHorizontal: 0, paddingHorizontal: 20, borderColor: theme.borderColor}]}>
                        ...
                        <Text style={styles.titleText}>{'Height'}</Text>
                        {
                            selectedHeightStatus === ''
                                ...
                                : <Icon type={'feather'} name={'check'} size={25} color={theme.blueColor} style={{fontSize: 25, color: theme.blueColor}} onPress={()=>onClose(selectedHeightStatus)}/>
                        }
                    </View>
                    <View style={[styles.itemView, {marginHorizontal: 0, paddingHorizontal: 20, paddingVertical: 0, marginTop: 20, borderBottomWidth: 0}]}>
                        <Text style={[styles.titleText, {color: theme.blueColor}]}>{'ft/inches'}</Text>
                        <Text style={[styles.titleText, {color: theme.blueColor}]}>{'cm'}</Text>
                    </View>
                    <FlatList

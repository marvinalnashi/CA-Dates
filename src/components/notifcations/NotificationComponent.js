...
class NotificationComponent extends Component {
    ...
    onRequestStatusPress = (status) => {
        ...
        updateActivitiesRequestStatus(relationship_id, status);
    ...
    renderItem = () => {
        ...
        const {user, notification_type, activitiesKey, address, createdAt, request_status} = item;

        if (notification_type === 'matches') {
            return <View style={[styles.nameView]}>
                <Text style={[styles.likeText, {color: theme.secondaryColor}]}>Congratulations!, You are a match with
        ...
        let getSubTitle = ` asked you on a ${getActivitiesTitle(activitiesKey).title.toLowerCase()} date`;
        return <View style={[styles.nameView]}>

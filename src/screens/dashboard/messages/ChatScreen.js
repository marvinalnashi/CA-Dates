...
import {addMessageInActivities, updateLatestMessageInActivities} from '../../../actions/activitiesAction';
...
import {conversationsCollection, activitiesRequestCollection} from '../../../config/firestore';
...
class ChatScreen extends React.Component {
    ...
    getDataFromActivities = () => {
        const {activities_id, user} = this.getConversationData();
        this.getAllMessagesFromActivities(activities_id, user);
    };

    getAllMessagesFromActivities = (activitiesId, otherUser) => {
    ...
    getData = () => {
        if (this.getType() === 'activities')
            this.getDataFromActivities();
        else
    ...
    onSend(messages = []) {
        ...
        if (this.getType() === 'activities') {
            const {activities_id} = this.getConversationData();
            if (messages.length > 0) {
                let parameter = messages[0];
                parameter.messageType = 'text';
                this.uploadDataInFirestore(activities_id, parameter);
            }
        } else  {
            const {matches_id} = this.getConversationData();
            if (messages.length > 0) {
    ...
    uploadDataInFirestore = (id, parameter) => {
        if (this.getType() === 'activities') {
            updateLatestMessageInActivities(id, parameter);
            addMessageInActivities(id, parameter);
        } else  {
        ...
        return (
            <Send {...props}>
                <View style={{marginRight: 10, marginBottom: 10}}>
                    <Icon type={'feather'} name={'send'} size={28} color={BLUE} style={{color: BLUE, fontSize: 28}}/>
                </View>
            </Send>
        );
    }
    ...
    renderMessageVideo(props) {
        ...
        return <View style={{backgroundColor: props.theme.blueColor, borderRadius: 15}}>
            <Video
                ref={r => {this.player = r;}}

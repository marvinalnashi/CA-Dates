...
import {updateActivitiesRequestStatus} from '../../../actions/activitiesAction';
...
class ActivitiesDetailScreen extends Component {
    ...
    onRequestStatusPress = (status) => {
        ...
        const { activities_id } = params.item;
    ...
    render() {
        ...
        let params = route.params;
        const { user, date, address, note, activitiesKey, request_status } = params.item;
        const {title} = getActivitiesTitle(activitiesKey);
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}
                        ...
                        <View style={[styles.innerView, {padding: 20}]}>
                            <View style={[styles.userView, {backgroundColor: theme.secondaryColor}]}>
                                 ...
                                 <FastImage source={require('../../../assets/activitiesphotogradient.png')} style={{width: null, height: HEIGHT_RATIO(.45), position: 'absolute', bottom: 0, left: 0, right: 0}}/>
                            ...
                            <View style={{marginTop: 15}}>
                                ...
                                <Text style={[styles.timeText, {marginTop: 5, color: theme.primaryColor}]}>{moment.unix(date).local().format('ddd, D MMM YYYY, hh:mm a')}</Text>
                            </View>
                            <View style={{marginTop: 15}}>
                                <View style={{flexDirection: 'row', alignItems: 'center'}}>
...
export default connect(mapStateToProps)(ActivitiesDetailScreen);
